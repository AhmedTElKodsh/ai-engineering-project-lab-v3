...
Teaching moves are defined in [teaching guide](../../../docs/TEACHING_GUIDE.md) and arranged into shapes in [lesson and interaction templates](../../../docs/LESSON_TEMPLATE.md). Read that routing table, pick the shape from the learner's most recent event, and read only the chosen shape. Those two files own the rules; they are deliberately not restated here.
