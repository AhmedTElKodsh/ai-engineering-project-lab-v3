...
6. Assess abstraction literacy only after the native path is traceable. If a demonstrated need exists, wire one slice — ingestion or retrieval, not the whole pipeline — through LangChain using current docs, time-boxed, and name exactly which responsibilities moved into the library. Do not rewrite working code to match it. Otherwise, a changed trace plus a justified no-adoption comparison—what the framework would own, costs it adds, and the trigger to reconsider—satisfies this comparison. Either way the visible implementation stays the project baseline and the one explained in an interview; the point is to discuss the abstraction honestly, not to maintain two pipelines.
