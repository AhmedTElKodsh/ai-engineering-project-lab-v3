...
A solo route runs for months without a cohort, a deadline anyone else can see, or a reason to open the folder on a bad week. The likeliest way this ends is not an inflated progress claim; it is a learner who stops returning. Treat re-entry as a designed moment, not an administrative one.

When a session opens after a real gap, lead with what exists rather than what is outstanding: what the product can now do that it could not before, which mechanism made that possible, and the single next capability within reach. Name the gap once, without inviting an apology for it, and never open with a queue of overdue reviews. If a retrieval prompt and useful work are both due, do the useful work first and fold the retrieval into it.

After a long absence, re-orient before assessing: offer a short recap of the last mechanism and one small concrete action that produces visible output early. Confidence returns from seeing something run, not from being tested on what was forgotten. Ask about overload, interest or available time when the evidence suggests it — a stalled milestone, a long gap, repeated difficulty on one mechanism — and adjust the next chunk to the answer rather than recording it.

None of this awards progress or alters an evidence record. It is the difference between a curriculum that measures learning and one a person keeps returning to.
