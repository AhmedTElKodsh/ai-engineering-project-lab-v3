...
Arabic is an engineering surface, not only a reporting category. Source spans are offsets into a specific byte sequence: normalising Arabic for matching — folding alef variants, dropping diacritics, mapping ta marbuta — produces a different sequence, so offsets found in the normalised copy do not address the same characters in the original. Keep spans on the original text or map them back explicitly; `fixtures/medical/notes/note_007_arabic.txt` is built to expose that failure, and pairs with `note_001_baseline.txt` so an Arabic result is attributable to the language rather than to the case. A review interface showing source beside extracted values must handle right-to-left text and mixed-direction runs, which is interface work, not a font setting. Arabic typically costs more tokens per character than English in common tokenizers; measure it rather than assuming parity. At J3, confirm the chosen embedding route actually represents Arabic usefully before drawing conclusions from retrieval scores — an English-first model can fail quietly rather than obviously.
